  ...
  return (
    <aside className="hidden lg:flex flex-col w-72 bg-white border-r border-[#e5e5e5] h-screen sticky top-0 max-h-screen">
      ...
      <nav className="flex-1 overflow-y-auto p-4 space-y-1">
        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              {isActive && (
                <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-6 bg-[#0070f3] rounded-r-full" />
              )}
      ...
      <div className="p-4 border-t border-[#e5e5e5]">
        <div className="flex items-center gap-3 p-3 rounded-lg bg-[#fafafa] border border-[#e5e5e5] hover:bg-[#f5f5f5] transition-all duration-200 cursor-pointer group">
          <div className="relative">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-[#171717] truncate">User</p>
            <p className="text-xs text-[#a3a3a3] truncate">Pro Account</p>
          </div>
        </div>
      </div>
    </aside>
