      ...
      <div className="flex items-center justify-around h-16 px-2">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              {isActive && (
                <div className="absolute top-0 left-1/2 -translate-x-1/2 w-10 h-0.5 bg-[#171717] rounded-b" />
              )}
              <Icon className={`h-5 w-5 transition-all duration-200 ${isActive ? 'scale-110' : ''}`} />
              <span className={`text-[10px] font-semibold tracking-wide ${isActive ? 'text-[#171717]' : 'text-[#a3a3a3]'}`}>
                {item.label}
              </span>
            </Link>
          );
